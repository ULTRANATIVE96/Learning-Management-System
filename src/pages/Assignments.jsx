...
export default function Assignments() {
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group flex-1 md:flex-none">
            ...
            <input 
              ...
              className="pl-11 pr-4 py-2.5 w-full md:w-64 bg-slate-900 border border-slate-800 rounded-xl text-sm focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all"
            />
          </div>
          <button 
            onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card hover-scale">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">To Do</p>
        ...
        <div className="glass-card hover-scale">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-emerald-50 bg-emerald-500/20 flex items-center justify-center text-emerald-600 text-emerald-400">
        ...
        <div className="glass-card hover-scale">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-rose-50 bg-rose-500/20 flex items-center justify-center text-rose-600 text-rose-400">
      ...
      <div className="glass-card p-0 overflow-hidden border-slate-800/50">
        <div className="px-6 py-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/50 bg-slate-900/50">
          <div className="flex items-center gap-3">
            <Filter className="w-4 h-4 text-slate-400" />
            <span className="text-sm font-bold text-slate-300">All Assessments</span>
          </div>
            ...
            {['Active', 'Submitted', 'Graded'].map((tab) => (
              <button 
                ...
                className={cn(
                  ...
                  tab === activeTab ? "bg-indigo-600 text-white shadow-md" : "text-slate-400 hover:text-slate-400"
                )}
              >
                {tab}
              </button>
        ...
        <div className="overflow-x-auto">
          {filteredAssignments.length === 0 ? (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] bg-slate-950/30 bg-slate-900/30">
                ...
                {filteredAssignments.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-900/40 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:bg-indigo-900/20 group-hover:text-indigo-600 transition-all">
                          <FileText className="w-5 h-5" />
                        </div>
                        <div>
                          <p className="font-bold text-slate-100 group-hover:text-indigo-600 transition-colors">{item.title}</p>
                    ...
                    <td className="px-8 py-6">
                      <span className="px-3 py-1 bg-slate-800 rounded-lg text-[10px] font-bold text-slate-500">{item.weight}</span>
                    </td>
                    <td className="px-8 py-6">
                      <span className={cn(
                        "px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm",
                        item.status === 'Submitted' ? "bg-emerald-50 text-emerald-700 bg-emerald-500/10 text-emerald-400" :
                        item.status === 'Late' ? "bg-rose-50 text-rose-700 bg-rose-500/10 text-rose-400" :
                        item.status === 'Graded' ? "bg-indigo-50 text-indigo-700 bg-indigo-500/10 text-indigo-400" :
                        "bg-amber-50 text-amber-700 bg-amber-500/10 text-amber-400"
                      )}>
                        {item.status}
                      </span>
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button 
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-slate-900 bg-slate-800 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-600 transition-all shadow-lg hover:shadow-indigo-600/20 group/btn"
                        >
